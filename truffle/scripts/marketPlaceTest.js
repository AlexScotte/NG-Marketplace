...
module.exports = async (deployer, network) => {
    ...
    console.log("Treasure balance: " + (await guardianStuffInstance.balanceOf(treasureGuardianInstance.address, itemId1)).toNumber());
    console.log("Treasure balance: " + (await guardianStuffInstance.balanceOf(treasureGuardianInstance.address, itemId2)).toNumber());

    console.log("Transfert item 1 from the treasure contract to the account1");
    await treasureGuardianInstance.safeTransferFrom(treasureGuardianInstance.address, account1, itemId1, 10, []);

    console.log("Get new balance of treasure contract and account1");
    console.log("Treasure balance: " + (await guardianStuffInstance.balanceOf(treasureGuardianInstance.address, itemId1)).toNumber());
    console.log("account1 balance: " + (await guardianStuffInstance.balanceOf(account1, itemId1)).toNumber());


    console.log("Give approval to the marketplace to list items of account1");
    await guardianStuffInstance.setApprovalForAll(auctionHouseInstance.address, true, { from: account1 });
    ...
    await auctionHouseInstance.listItem(itemId1, web3.utils.toWei('1', 'ether'), { from: account1, value: listingFee });

    // console.log("Get new balance after listing")
    // console.log("account1 balance: " + (await guardianStuffInstance.balanceOf(account1, itemId1)).toNumber());
    // console.log("marketplace balance: " + (await guardianStuffInstance.balanceOf(auctionHouseInstance.address, itemId1)).toNumber());
    ...
    // await auctionHouseInstance.executeSale(0, { from: account2, value: web3.utils.toWei('1', 'ether') });
    // console.log("New balance after the sell");
    // console.log("account1 balance: " + (await guardianStuffInstance.balanceOf(account1, itemId1)).toNumber());
    // console.log("account2 balance: " + (await guardianStuffInstance.balanceOf(account2, itemId1)).toNumber());
    // console.log("marketplace balance: " + (await guardianStuffInstance.balanceOf(auctionHouseInstance.address, itemId1)).toNumber());

    // console.log("Give approval to the market place to list item of account 2");
    // await guardianStuffInstance.setApprovalForAll(auctionHouseInstance.address, true, { from: account2 });
    // console.log("Balance before listing");
    // console.log("account2 balance: " + (await guardianStuffInstance.balanceOf(account2, itemId1)).toNumber());
    // await auctionHouseInstance.listItem(itemId1, web3.utils.toWei('1', 'ether'), { from: account2, value: listingFee });
