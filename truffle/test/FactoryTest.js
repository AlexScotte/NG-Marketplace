const ForgeMaster = artifacts.require("../contracts/Forge/ForgeMaster.sol");
...
contract("***** Factory *****", accounts => {
    ...
    //         factory = await ForgeMaster.new({ from: account0 });
    //     });

    //     it("...Should return address of the new created collection", async () => {

    //         await factory.forgeCollection("Test collection", { from: account0 });
    ...
    describe("Token deployment", function () {
        ...
        beforeEach(async function () {
            ...
            factory = await ForgeMaster.new({ from: account0 });
            await factory.forgeCollection("Test collection", { from: account0 });
            collectionAddress = await factory.collectionAddress.call();
        });

        it("...Should have deployed the new token", async () => {

            const receipt = await Stuff.at(collectionAddress);
            console.log(collectionAddress);
            console.log(receipt.address);


            // const MyContract = await Stuff.new(collectionAddress);
            // console.log(MyContract);

            // const receipt = await Stuff.deployed(collectionAddress);
            // console.log(receipt);
            // await factory.forgeCollection("Test collection", { from: account0 });
            // const receipt = await factory.collectionAddress.call({ from: account0 });
            // expect(receipt).not.equal(ZERO_ADDRESS);
        ...
        // it("...Should revert if token initialized at the wrong address", async () => {
